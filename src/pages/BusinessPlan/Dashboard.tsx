...
export default function BPDashboard() {
  const { data, getBreakEvenYear, getGrossMargin, isLoading } = useProfitLoss();
  ...
  const breakEven = getBreakEvenYear();

  const kpis = [
    { title: `CA Total (${settings.bp_years} ans)`, value: formatCurrency(data.grandTotal.revenue), icon: TrendingUp, color: "text-success" },
    { title: "Marge Brute", value: `${getGrossMargin().toFixed(1)}%`, icon: Target, color: "text-primary" },
    { title: "Résultat Net Total", value: formatCurrency(data.grandTotal.netResult), icon: PiggyBank, color: data.grandTotal.netResult >= 0 ? "text-success" : "text-destructive" },
    { title: "Rentabilité", value: breakEven ? `Année ${breakEven}` : "–", icon: Calendar, color: "text-secondary" },
  ];
  ...
  const chartData = data.years.map((year, i) => ({
    ...
    Charges: data.totals.fixedExpenses[i] + data.totals.personnelCosts[i] + (data.totals.variableExpenses[i] || 0),
    Résultat: data.totals.netResult[i],
  }));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Vue d'ensemble de vos projections financières sur {settings.bp_years} ans</p>
      </div>
      ...
      <Card>
        <CardHeader><CardTitle>Évolution Revenus vs Charges par Année</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={chartData}>
              ...
              <Bar dataKey="Résultat" fill="hsl(221, 83%, 53%)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
