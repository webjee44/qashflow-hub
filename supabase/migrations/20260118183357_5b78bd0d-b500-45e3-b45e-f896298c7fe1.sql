...
DROP FUNCTION IF EXISTS public.get_superadmin_org_stats();

-- Add is_demo column to organizations (if not already added)
ALTER TABLE public.organizations 
ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false;

-- Create index for filtering demo accounts
CREATE INDEX IF NOT EXISTS idx_organizations_is_demo ON public.organizations(is_demo) WHERE is_demo = true;

-- Recreate the get_superadmin_org_stats function with is_demo
